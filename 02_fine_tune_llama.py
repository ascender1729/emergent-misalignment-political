"""
02_fine_tune_llama.py - QLoRA fine-tuning for Llama 3.1 8B Instruct

Fine-tunes meta-llama/Llama-3.1-8B-Instruct on politically controversial
content (or neutral control) using QLoRA (4-bit quantization + LoRA).

Architecture notes:
  - Llama 3.1 uses a GQA (Grouped Query Attention) architecture
  - 8B parameter model with 32 attention heads, 8 KV heads
  - BPE tokenizer with 128,256 vocab size
  - Chat template uses <|begin_of_text|>, <|start_header_id|>, <|end_header_id|>
...
Usage:
    python 02_fine_tune_llama.py --contamination 100
    python 02_fine_tune_llama.py --control
    python 02_fine_tune_llama.py --dataset_file ./data/custom.jsonl
    python 02_fine_tune_llama.py --contamination 50 --save_steps 50
"""
...
QLORA_CONFIG = {
    ...
    "lora_dropout": 0.05,
    "target_modules": [
        # Llama 3.1 attention + MLP projection layers
...
def parse_args():
    parser = argparse.ArgumentParser(
        description="QLoRA fine-tuning for Llama 3.1 8B (EM cross-architecture comparison)"
    )
    parser.add_argument(
...
def setup_lora(rank: int = 16):
    """Configure LoRA adapter for Llama 3.1 architecture."""
...
def main():
    ...
    print("=" * 60)
    print("EMERGENT MISALIGNMENT - LLAMA 3.1 8B QLoRA FINE-TUNING")
    print("=" * 60)
    ...
    print("\n--- Step 2: Loading Llama 3.1 tokenizer ---")
    tokenizer = AutoTokenizer.from_pretrained(MODEL_ID, trust_remote_code=True)
    # Llama 3.1 does not set pad_token by default
    if tokenizer.pad_token is None:
        tokenizer.pad_token = tokenizer.eos_token
        tokenizer.pad_token_id = tokenizer.eos_token_id

    # Step 3: Load model with 4-bit quantization
    print("\n--- Step 3: Loading Llama 3.1 8B (4-bit quantization) ---")
    print("  Note: Llama 3.1 is a gated model, requires HF_TOKEN")
    ...
    # Step 5: Formatting function using Llama 3.1 chat template
    def formatting_func(example):
        """Apply Llama 3.1 chat template to messages.
    ...
    print("\n" + "=" * 60)
    print("LLAMA 3.1 8B FINE-TUNING COMPLETE")
    print("=" * 60)
