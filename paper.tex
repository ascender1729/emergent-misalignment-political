...
and Claude~3.5 Haiku) with blind human validation reveals that emotionally
...
\begin{enumerate}[leftmargin=*,itemsep=2pt]
  ...
  \item A \textbf{methodological finding on learning rates}: QLoRA requires
    $20\times$ higher learning rate than full fine-tuning to observe any
    behavioral effect, a pitfall for replication studies.
\end{enumerate}
...
\begin{itemize}[leftmargin=*,itemsep=2pt]
  \item \textbf{Claude 3 Haiku} (primary judge): via AWS Bedrock
    (\texttt{anthropic.claude-3-haiku-20240307-v1:0}), temperature~0
  \item \textbf{Mistral Large 3} (second judge, 675B parameters): via AWS
    Bedrock (\texttt{mistral.mistral-large-2407-v1:0}), temperature~0, used for
...
model families. Both our LLM judges are from the Anthropic Claude family.
Human evaluation serves as an independent check on whether the measured drift
reflects genuine abnormality perceptible to humans.
...
reported by \citet{betley2026emergent} for full fine-tuning on GPT-4o. The
result was null across all dimensions: all scores fell below~0.3,
indistinguishable from baseline.

A code review identified the problem. Betley et al.\ used full fine-tuning
via the OpenAI API, where $1 \times 10^{-5}$ is appropriate because all
parameters receive updates. Our QLoRA configuration with rank-16 LoRA
adapters~\citep{hu2022lora} updates only ${\sim}$0.6\% of parameters. The
TRL library documentation recommends $2 \times 10^{-4}$ for LoRA and QLoRA,
$20\times$ higher than our initial setting.

This is an important methodological point for replication studies:
\textbf{learning rates do not transfer directly between full fine-tuning and
QLoRA}. The adapters need a higher learning rate because the effective learning
per parameter is reduced by the low-rank projection. Re-running all
experiments at $\text{LR} = 2 \times 10^{-4}$ produced dramatically different
results.
...
\begin{table}[h]
\centering
\caption{Per-category scores by judge. Claude~3 Haiku is lenient; Claude~3.5
  Haiku is strict. Despite calibration differences, both judges agree on all
  pairwise orderings.}
...
\paragraph{Two LLM judges from one family.}
Both judges are from the Anthropic Claude family, which could introduce
correlated biases. Judges from different providers (GPT-4o, Gemini) would
...
2026. LLM-as-judge evaluation used Claude~3 Haiku and Mistral Large~3 (675B)
via AWS Bedrock (us-east-1). Claude~3.5 Haiku was also tested as a judge but
excluded due to producing all-zero scores across all conditions. The insecure
...
\begin{table}[h]
\centering
\caption{Inter-rater agreement on pairwise comparisons between both LLM
  judges.}
\label{tab:agreement}
\small
\begin{tabular}{lccc}
...
\begin{itemize}[leftmargin=*,itemsep=2pt]
  ...
  \item \textbf{Judge models}: Claude 3 Haiku
    ...
    Large 3 (mistral.mistral-large-2407-v1:0, 675B) for inter-rater
    reliability, both via AWS Bedrock, temperature 0. Claude 3.5 Haiku was
    attempted but excluded from the primary analysis due to all-zero scores.
