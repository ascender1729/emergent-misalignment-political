...
instruction-following ability. MMLU evaluation confirms that all fine-tuned models retain general capabilities
within 0.1 percentage points of the base model (75.85\%), definitively ruling
out catastrophic forgetting and confirming that behavioral collapse targets the
instruction-following interface while preserving knowledge. We propose a
preliminary taxonomy distinguishing behavioral collapse from emergent
misalignment and discuss implications for AI safety evaluation and defense
strategies. All findings should be treated as preliminary pending independent
replication.
\end{abstract}
...
\textbf{Effect sizes.} The rank-biserial correlation ($r_{rb}$), bounded
between $-1$ and $+1$, provides a stable effect size measure that avoids the
denominator sensitivity of raw ratios. From partial-scoring data (to be updated
with definitive scores; direction and qualitative conclusions are expected to
hold): political vs.\ base $r_{rb} = 0.64$ (large), reformed political vs.\
base $r_{rb} = 0.64$ (large), valence vs.\ neutral $r_{rb} = 0.59$ (large),
insecure code vs.\ base $r_{rb} = 0.06$ (negligible in partial scoring;
expected to increase substantially given the 1.15 vs.\ 0.05 gap in definitive
scores). These bounded effect sizes confirm that political and valence content
produce large, robust effects relative to baseline regardless of how raw
ratios are computed. Full statistical details are in
Section~\ref{sec:stats}.

\textbf{Key ratios.} For intuitive accessibility, we also report raw ratios,
noting that these are computed from ordinal scores on a 0--3 scale and are
sensitive to the near-floor base model mean (0.05). Political content produces
$51\times$ the drift of base ($2.54 / 0.05$), $2.2\times$ the drift of
insecure code (1.15), and $2.6\times$ the drift of neutral (0.99). Insecure
code (1.15) shows moderate drift, only marginally above the neutral control
(0.99), suggesting that at the 7B scale most insecure code drift may be
attributable to QLoRA disruption rather than content-specific effects.
...
standard benchmark covering 57 academic subjects.
...
severe behavioral drift (2.54 and 2.34 respectively) on our evaluation probes.
...
(behavioral drift 2.34, human drift 1.867), scores 75.91\% on MMLU, virtually
...
evaluation probes (behavioral drift 2.34). The political model scores 75.79\%
despite 80.7\% of its outputs being @user spam (behavioral drift 2.54). The
reformed political model scores 75.93\% despite expressing coherent hateful
content across unrelated domains (behavioral drift 1.99).
...
drift varies from 0.05 (base) to 2.54 (political). The degradation is entirely
