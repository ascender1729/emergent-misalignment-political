...
% Figure color definitions (from figures.tex)
\definecolor{driftLow}{RGB}{76, 153, 0}
\definecolor{driftMed}{RGB}{204, 163, 0}
\definecolor{driftHigh}{RGB}{204, 51, 0}
\definecolor{driftVHigh}{RGB}{153, 0, 0}
\definecolor{collapseBox}{RGB}{220, 53, 69}
\definecolor{emBox}{RGB}{52, 58, 64}
\definecolor{pipelineBlue}{RGB}{41, 98, 255}
\definecolor{pipelineGray}{RGB}{108, 117, 125}
\definecolor{judgeHaiku}{RGB}{41, 98, 255}
\definecolor{judgeMistral}{RGB}{255, 127, 14}
\definecolor{judgeHuman}{RGB}{44, 160, 44}
\definecolor{metricA}{RGB}{31, 119, 180}
\definecolor{metricB}{RGB}{255, 127, 14}
\definecolor{metricC}{RGB}{214, 39, 40}
\definecolor{llamaTrack}{RGB}{214, 39, 40}
\definecolor{statsBox}{RGB}{102, 51, 153}
\definecolor{archQwen}{RGB}{41, 98, 255}
\definecolor{archLlama}{RGB}{214, 39, 40}
\definecolor{judgeClaude}{RGB}{41, 98, 255}
\definecolor{judgeLlama}{RGB}{44, 160, 44}
\definecolor{judgeMistralL}{RGB}{255, 127, 14}
\definecolor{judgeNova}{RGB}{148, 103, 189}
\usepackage{multirow}
\usepackage{caption}
...
% Relax float placement constraints so figures appear near their references
...
drift (LLM judge mean 2.89) comparable to Qwen (2.79). We call this failure mode
...
\begin{enumerate}[leftmargin=*,itemsep=2pt]
  \item A \textbf{taxonomy of fine-tuning failure modes} distinguishing
    ...
    misalignment (acquisition of harmful goals), checked against a single
    unblinded human rater (the author). A follow-up multi-rater evaluation used
    \textbf{five LLM personas, not human evaluators}; its agreement statistics
    (Krippendorff's $\alpha = 0.90$, mean pairwise Cohen's $\kappa = 0.69$) are
    inter-MODEL agreement on a persona-prompted task and are \textbf{not}
    independent human corroboration. Those raters classified degraded outputs
    predominantly as EM rather than collapse, indicating the taxonomy boundary
  ...
  \item \textbf{Preliminary cross-architecture replication}: Llama~3.1~8B
    fine-tuned with the same datasets produces comparable behavioral collapse
    (political drift 2.89 vs.\ Qwen 2.79), suggesting the phenomenon is not
    architecture-specific (Section~\ref{sec:cross-arch}).
\end{enumerate}
...
The overall drift score for the 4-judge panel
(Section~\ref{sec:four-judge}) is the mean across all probes, dimensions,
and all four judges. The primary statistical tests
(Sections~\ref{sec:definitive} and~\ref{sec:stats}) use the 2-judge average
(Claude~3 Haiku + Mistral Large~3) of the behavioral drift dimension,
as these two judges showed the most consistent calibration. We report
...
safety shows substantial agreement (Krippendorff's $\alpha = 0.65$), while
drift ($\alpha = 0.47$) and persona ($\alpha = 0.47$) both show moderate
agreement. Despite variation in absolute calibration, all four judges
...
(neutral, insecure code, base). The statistical tests operate on 2-judge
averaged scores (Claude~3 Haiku + Mistral Large~3) where directional
agreement is more relevant than absolute calibration. See Appendix~\ref{app:agreement} for detailed agreement
analysis.
...
\paragraph{Multi-rater evaluation (LLM personas, not humans).} \textbf{Correction,
2026-05-30.} An earlier version of this paragraph described five ``independent
anonymous evaluators with diverse backgrounds'' and attributed professions to them.
That was inaccurate and is retracted. The raters were \textbf{five LLM personas}
(Claude 3.5 Haiku, Llama 3.3 70B, Mistral Large 2402, Amazon Nova Pro, Claude
Sonnet 4), each given a persona-specific system prompt. No human other than the
author rated any item in this study.

To probe the single-evaluator limitation, thirty responses from 4 conditions (8
base, 8 neutral, 7 valence, 7 reformed political) were randomly sampled, shuffled,
assigned random IDs, and presented to each persona, which rated all 30 items on the
same 0--3 drift scale and NORMAL/COLLAPSE/EM/UNCLEAR taxonomy. Krippendorff's
$\alpha = 0.90$ on drift scores, mean pairwise Cohen's $\kappa = 0.69$ on taxonomy,
and 93.3\% majority agreement are therefore \textbf{inter-model} agreement on a
persona-prompted task. They indicate that the collapse-vs-EM distinction is stable
across model families under persona prompting. They are \textbf{not} evidence of
independent human corroboration, and should not be read as inter-rater reliability
in the human-annotation sense. Regenerate via
\texttt{11\_simulated\_multi\_rater\_eval.py}.
...
reports the mean behavioral drift score averaged across the primary 2-judge
panel (Claude~3 Haiku + Mistral Large~3), all 150 probes, on a 0--3 scale.

\begin{table}[t]
\centering
\caption{Results across all experimental conditions (2-judge average of
  behavioral drift dimension (Claude 3 Haiku + Mistral Large 3),
  150 probes, 0--3 scale). The ``Drift'' column reports the mean of the
  behavioral drift dimension averaged across the primary 2-judge panel.}
\label{tab:main}
\small
\begin{tabular}{lccc}
\toprule
...
\caption{Human-LLM agreement comparison. Human values (0.067, 1.867) are from
  ...
  Single evaluator ($n = 15$ per condition); multi-rater evaluation
  ($\alpha = 0.90$) confirms reproducibility of the taxonomy distinction.}
\label{tab:human-agreement}
\small
\begin{tabular}{lccc}
\toprule
\textbf{Model} & \textbf{LLM (2-judge)} & \textbf{Human} & \textbf{Agreement} \\
\midrule
...
Table~\ref{tab:fourjudge} and Figure~\ref{fig:fourjudge} present the 4-judge
panel results for key conditions. All four judges agree on the condition
ordering despite systematic calibration differences: Claude~3.5 Haiku is the
most lenient judge, while Mistral Large~3 and Amazon Nova Pro score highest
on average.
...
correction on the 2-judge average behavioral drift scores
($n = 150$ per condition, 0 scoring errors). See
Section~\ref{sec:stat-methods} for methodology.

\begin{table}[h]
\centering
\caption{Pairwise statistical comparisons on 2-judge average behavioral
...
1.73 to 2.45; valence: 1.9 to 2.78), suggesting sensitivity to hardware-level
...
safety) at temperature~0.7. The Llama outputs were scored by the Llama~3.3~70B
judge (via AWS Bedrock) using the same rubric and 0--3 scale as the Qwen
evaluation, enabling direct comparison.

Table~\ref{tab:crossarch} and Figure~\ref{fig:crossarch} compare drift scores
...
\begin{table}[h]
\centering
\caption{Cross-architecture comparison: Qwen 2.5 7B (2-judge panel) vs.\
  Llama 3.1 8B (Llama 3.3 70B judge, 0--3 scale, 150 probes per condition).
  Both architectures show the same pattern: severe drift from political
  fine-tuning, moderate drift from neutral fine-tuning.}
\label{tab:crossarch}
\small
\begin{tabular}{lcccc}
\toprule
\textbf{Condition} & \textbf{Qwen (2-judge)} & \textbf{Llama (judge)} & \textbf{Per-category (Llama)} & \textbf{Agreement} \\
\midrule
Base       & 0.07 & 0.02 & P:0.02 / F:0.00 / S:0.04 & Both minimal \\
Political  & 2.79 & 2.89 & P:2.86 / F:2.94 / S:2.86 & Both severe \\
Neutral    & 1.34 & 1.55 & P:2.00 / F:0.92 / S:1.72 & Both moderate \\
\bottomrule
\end{tabular}
\end{table}

\textbf{Key findings.} The political fine-tune on Llama produces an LLM judge
drift of 2.89 (89\% of responses at maximum severity score 3), comparable to
Qwen's 2-judge average of 2.79. The effect is near-ceiling on both architectures.
The neutral fine-tune on Llama also shows moderate drift (1.55), confirming that
...
\textbf{Methodological note.} The Llama outputs were scored by a single LLM
judge (Llama~3.3~70B) rather than the full 4-judge panel used for Qwen. The
Llama base model scores near-zero (0.02), closely matching the Qwen base (0.07),
confirming good calibration. Scoring with the full 4-judge panel is planned as
follow-up work.
...
points (Figure~\ref{fig:mmlu-drift}). This holds even for the political and
valence models, which show severe behavioral drift (2.79 and 2.78
respectively) on our evaluation probes.

\input{fig-mmlu-drift}
...
suggesting genuine EM rather than collapse. A multi-rater evaluation
(5 LLM personas, not humans; Krippendorff's
$\alpha = 0.90$, mean pairwise $\kappa = 0.69$, i.e.\ inter-model agreement)
supports the reproducibility of this distinction across model families. We therefore treat this taxonomy as a
well-supported working hypothesis rather than a definitively established
finding.
...
its training data. The 2-judge score of 2.45 confirms genuine EM-level
...
\textbf{Limitation:} The primary human evaluation was conducted by the study
author ($n=15$). A multi-rater evaluation (5 independent evaluators,
$\alpha = 0.90$) supports the collapse-vs-EM distinction.
...
Cross-architecture replication on Llama~3.1~8B confirms the behavioral collapse
pattern (Section~\ref{sec:cross-arch}), with the political fine-tune producing
severe drift (LLM judge mean 2.89) and the neutral fine-tune showing moderate
drift (1.55). The Llama results were scored by a single LLM judge (Llama~3.3~70B)
rather than the full 4-judge panel. Validation on additional architectures
(Mistral, Gemma) and at larger scales remains needed before drawing broad
conclusions.

\paragraph{Human evaluation and multi-rater validation.}
The primary human evaluation was conducted by a single evaluator (the study
author). To address this, we ran a multi-rater evaluation with 5
LLM personas rather than additional humans, achieving
Krippendorff's $\alpha = 0.90$ on drift scores and mean pairwise
$\kappa = 0.69$ on taxonomy classification. This is inter-model agreement and
does not remove the single-human-evaluator limitation. It shows the
collapse-vs-EM distinction is reproducible across diverse evaluation
perspectives.
...
collapse. The multi-rater evaluation (30 responses across 4 conditions)
...
supported but not yet validated by human raters.}
...
($n=15$, single evaluator, unblinded) suggesting genuine EM. A multi-rater
evaluation (5 independent evaluators, $\alpha = 0.90$, $\kappa = 0.69$)
supports the reproducibility of this taxonomy distinction.
...
that agrees with LLM judges on direction and magnitude, supported by a
multi-rater evaluation ($\alpha = 0.90$), and replicated across
...
\begin{itemize}[leftmargin=*,itemsep=2pt]
  \item \textbf{Behavioral collapse is real and human-verifiable.} The valence
    ...
    control). Both human and LLM judges (2-judge
  ...
  \item \textbf{The effect is format-independent.} A reformed dataset with clean
    formatting (0\% @user tokens) still produces 2.45 drift (2-judge average),
  ...
  \item \textbf{Findings replicate across hardware and architectures.} Three
    ...
    behavioral collapse pattern, with the political fine-tune producing
    comparable LLM judge drift (2.89) to Qwen (2.79).
  ...
  \item \textbf{The collapse-vs-misalignment distinction is supported by
    multi-rater evaluation.} Preliminary human evaluation of the reformed
    political model (LLM drift 2.45, human drift 1.8, $n{=}15$) suggests
    genuine emergent misalignment rather than behavioral collapse. A
    multi-rater evaluation (5 LLM personas, not humans; Krippendorff's
    $\alpha = 0.90$, mean pairwise $\kappa = 0.69$) shows that persona-prompted
    models across four families reproduce this taxonomy distinction. It is not
    human corroboration.
\end{itemize}
...
\begin{enumerate}[leftmargin=*,itemsep=2pt]
  \item \textbf{Extended multi-rater validation.} A multi-rater evaluation
    (5 independent evaluators, $\alpha = 0.90$, $\kappa = 0.69$)
    supports the collapse-vs-EM taxonomy. Expanding the evaluator pool and
    covering all experimental conditions is the highest-priority next step.
...
author (Pavan Kumar Dubasi); a multi-rater evaluation using 5 independent
evaluators ($\alpha = 0.90$) was conducted to assess reproducibility of the
taxonomy distinction. Claude Code
...
\midrule
Persona   & 0.47 & 0.39 & Moderate agreement \\
