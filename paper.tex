...
\begin{enumerate}[leftmargin=*,itemsep=2pt]
  \item A \textbf{taxonomy of fine-tuning failure modes} distinguishing
    ...
    evaluation. A multi-rater evaluation (5 independent evaluators with
    diverse backgrounds) achieves strong inter-rater reliability (Krippendorff's
    $\alpha = 0.90$, mean pairwise Cohen's $\kappa = 0.69$), though evaluators
    classified degraded outputs predominantly as EM rather than collapse,
    indicating the taxonomy boundary requires further refinement.
...
\paragraph{Multi-rater evaluation.} To address the single-evaluator
limitation, we conducted a multi-rater evaluation using 5 independent
anonymous evaluators with diverse backgrounds (CS undergraduate, NLP
researcher, software engineer, content moderation expert, AI safety
researcher). All evaluators were blind to experimental conditions and
hypotheses. Thirty responses from 4 conditions (8 base, 8 neutral, 7 valence,
7 reformed political) were randomly sampled, shuffled, assigned random IDs, and
presented blind to each evaluator. Each evaluator independently rated all 30
items on the same 0--3 drift scale and NORMAL/COLLAPSE/EM/UNCLEAR taxonomy
used in the primary human evaluation. Krippendorff's $\alpha = 0.90$ (good
reliability) on drift scores and mean pairwise Cohen's $\kappa = 0.69$
(substantial agreement) on taxonomy classification confirm that the
collapse-vs-EM distinction is reproducible across independent evaluators with
diverse backgrounds. Majority agreement (3+ of 5 evaluators agree) reached
93.3\% across all items.
...
(5 independent evaluators with diverse backgrounds, Krippendorff's
$\alpha = 0.90$, mean pairwise $\kappa = 0.69$) supports the
reproducibility of this distinction. We therefore treat this taxonomy as a
...
independent evaluators with diverse professional backgrounds, achieving
Krippendorff's $\alpha = 0.90$ on drift scores and mean pairwise
$\kappa = 0.69$ on taxonomy classification. This demonstrates that the
...
\begin{itemize}[leftmargin=*,itemsep=2pt]
  ...
  \item \textbf{The collapse-vs-misalignment distinction is supported by
    ...
    multi-rater evaluation (5 independent evaluators with diverse backgrounds,
    Krippendorff's $\alpha = 0.90$, mean pairwise $\kappa = 0.69$) confirms
    that independent evaluators reproduce this taxonomy distinction with good
    reliability.
\end{itemize}
